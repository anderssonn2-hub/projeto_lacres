...
function extrairUltimoLacreSequencialPreview($valor) {
...
try {
    ...
    if (isset($_POST['salvar_oficio_correios_preview_ajax'])) {
        ...
        $snapshotBruto = isset($_POST['snapshot_oficio']) ? (string)$_POST['snapshot_oficio'] : '';
        $datasJson = isset($_POST['datas_json']) ? (string)$_POST['datas_json'] : '';

        if ($usuario === '') {
            die(json_encode(array('success' => false, 'erro' => 'Responsável obrigatório para gravar o ofício.')));
        }

        $snapshot = json_decode($snapshotBruto, true);
        if (!is_array($snapshot) || !isset($snapshot['resumo']) || !is_array($snapshot['resumo'])) {
            die(json_encode(array('success' => false, 'erro' => 'Snapshot do ofício inválido.')));
        }

        $datasSql = json_decode($datasJson, true);
        if (!is_array($datasSql) || empty($datasSql)) {
            $datasSql = isset($snapshot['datas_filtro']) && is_array($snapshot['datas_filtro']) ? $snapshot['datas_filtro'] : array();
        }
        $datasNormalizadas = array();
        foreach ($datasSql as $dataItem) {
            $dataNormalizada = normalizarDataSql(trim((string)$dataItem));
            if ($dataNormalizada !== '') {
                $datasNormalizadas[$dataNormalizada] = $dataNormalizada;
            }
        }
        $datasSql = array_values($datasNormalizadas);
        if (empty($datasSql)) {
            $datasSql[] = date('Y-m-d');
        }
        ...
        $etiquetasSnapshot = array();
        foreach ($snapshot['resumo'] as $itemResumo) {
            if (!is_array($itemResumo)) {
                continue;
            }
            $posto = str_pad(preg_replace('/\D+/', '', (string)(isset($itemResumo['posto']) ? $itemResumo['posto'] : '')), 3, '0', STR_PAD_LEFT);
            if ($posto === '000' || $posto === '') {
                continue;
            }
            $postosSelecionados[$posto] = $posto;
            $lotesResumo = isset($itemResumo['lotes']) && is_array($itemResumo['lotes']) ? $itemResumo['lotes'] : array();
            $lotesNormalizados = array();
            foreach ($lotesResumo as $loteResumo) {
                $loteTexto = trim((string)$loteResumo);
                if ($loteTexto === '') {
                    continue;
                }
                $loteDigitos = preg_replace('/\D+/', '', $loteTexto);
                if ($loteDigitos !== '') {
                    $loteTexto = str_pad($loteDigitos, 8, '0', STR_PAD_LEFT);
                }
                $lotesNormalizados[$loteTexto] = $loteTexto;
            }
            ...
            $registroLinha = array(
                'posto' => $posto,
                'regional' => str_pad(preg_replace('/\D+/', '', (string)(isset($itemResumo['regional']) ? $itemResumo['regional'] : '')), 3, '0', STR_PAD_LEFT),
                'lacre_iipr' => extrairUltimoLacreSequencialPreview(isset($itemResumo['lacre_iipr']) ? $itemResumo['lacre_iipr'] : ''),
                'grupo_iipr' => isset($itemResumo['grupo_iipr']) ? trim((string)$itemResumo['grupo_iipr']) : '',
                'lacre_correios' => extrairUltimoLacreSequencialPreview(isset($itemResumo['lacre_correios']) ? $itemResumo['lacre_correios'] : ''),
                'grupo_correios' => isset($itemResumo['grupo_correios']) ? trim((string)$itemResumo['grupo_correios']) : '',
                'etiqueta_correios' => isset($itemResumo['etiqueta_correios']) ? trim((string)$itemResumo['etiqueta_correios']) : '',
                'qtd_total' => isset($itemResumo['qtd_total']) ? (int)$itemResumo['qtd_total'] : 0,
                'row_key' => isset($itemResumo['row_key']) ? trim((string)$itemResumo['row_key']) : ''
            );

            if ($registroLinha['etiqueta_correios'] !== '') {
                $etiquetasSnapshot[$registroLinha['etiqueta_correios']] = array(
                    'posto' => $posto,
                    'etiqueta' => $registroLinha['etiqueta_correios']
                );
            }

            foreach ($lotesNormalizados as $loteNormalizado) {
                $linhasPorLote[$posto . '|' . $loteNormalizado] = $registroLinha;
            }
        }

        if (empty($linhasPorLote)) {
            die(json_encode(array('success' => false, 'erro' => 'Nenhum lote válido foi encontrado na prévia.')));
        }

        $pdo->beginTransaction();
        try {
            $ultimoOficio = 0;
            $stmtUltimo = $pdo->query("SELECT id FROM ciDespachos WHERE grupo = 'CORREIOS' ORDER BY id DESC LIMIT 1");
            if ($stmtUltimo) {
                $ultimoOficio = (int)$stmtUltimo->fetchColumn();
            }

            $idDespacho = 0;
            $etiquetasAntigas = array();
            if ($modoOficio === 'novo') {
                $hash = sha1('CORREIOS|' . $datasStr . '|' . microtime(true) . '|' . mt_rand());
                $stmtInsDesp = $pdo->prepare("INSERT INTO ciDespachos (usuario, grupo, datas_str, hash_chave, ativo, obs) VALUES (?, 'CORREIOS', ?, ?, 1, NULL)");
                $stmtInsDesp->execute(array($usuario, $datasStr, $hash));
                $idDespacho = (int)$pdo->lastInsertId();
            } else {
                if ($idSobrescrever > 0) {
                    $stmtBuscaDesp = $pdo->prepare("SELECT id FROM ciDespachos WHERE id = ? AND grupo = 'CORREIOS' LIMIT 1");
                    $stmtBuscaDesp->execute(array($idSobrescrever));
                    $idDespacho = (int)$stmtBuscaDesp->fetchColumn();
                }
                if ($idDespacho <= 0) {
                    $idDespacho = $ultimoOficio;
                }
                if ($idDespacho > 0) {
                    $stmtEtiqAntigas = $pdo->prepare("SELECT DISTINCT etiqueta_correios FROM ciDespachoLotes WHERE id_despacho = ? AND etiqueta_correios IS NOT NULL AND etiqueta_correios <> ''");
                    $stmtEtiqAntigas->execute(array($idDespacho));
                    while ($rowEtiquetaAntiga = $stmtEtiqAntigas->fetch(PDO::FETCH_ASSOC)) {
                        $etiquetaAntiga = trim((string)$rowEtiquetaAntiga['etiqueta_correios']);
                        if ($etiquetaAntiga !== '') {
                            $etiquetasAntigas[$etiquetaAntiga] = $etiquetaAntiga;
                        }
                    }

                    $stmtUpdDesp = $pdo->prepare("UPDATE ciDespachos SET usuario = ?, datas_str = ?, ativo = 1, obs = NULL WHERE id = ?");
                    $stmtUpdDesp->execute(array($usuario, $datasStr, $idDespacho));

                    $stmtDelItens = $pdo->prepare("DELETE FROM ciDespachoItens WHERE id_despacho = ?");
                    $stmtDelItens->execute(array($idDespacho));
                    $stmtDelLotes = $pdo->prepare("DELETE FROM ciDespachoLotes WHERE id_despacho = ?");
                    $stmtDelLotes->execute(array($idDespacho));
                } else {
            ...
            $postosLista = array_values($postosSelecionados);
            $placePostos = implode(',', array_fill(0, count($postosLista), '?'));
            $sqlLotesOficio = "SELECT LPAD(CAST(c.posto AS UNSIGNED), 3, '0') AS posto,
                                      LPAD(CAST(COALESCE(r.regional, c.regional) AS UNSIGNED), 3, '0') AS regional,
                                      LPAD(CAST(c.lote AS UNSIGNED), 8, '0') AS lote,
                                 ...
                                 FROM ciPostosCsv c
                            LEFT JOIN ciRegionais r ON LPAD(CAST(r.posto AS UNSIGNED), 3, '0') = LPAD(CAST(c.posto AS UNSIGNED), 3, '0')
                                WHERE DATE(c.dataCarga) IN ($placeDatas)
                                  AND LPAD(CAST(c.posto AS UNSIGNED), 3, '0') IN ($placePostos)
                             GROUP BY LPAD(CAST(c.posto AS UNSIGNED), 3, '0'), LPAD(CAST(c.lote AS UNSIGNED), 8, '0')";
            $stmtLotesOficio = $pdo->prepare($sqlLotesOficio);
            $stmtLotesOficio->execute(array_merge($datasSql, $postosLista));

            $registrosEncontrados = array();
            while ($rowLoteOficio = $stmtLotesOficio->fetch(PDO::FETCH_ASSOC)) {
                $chaveLote = $rowLoteOficio['posto'] . '|' . $rowLoteOficio['lote'];
                if (!isset($linhasPorLote[$chaveLote])) {
                    continue;
                }
                $configLinha = $linhasPorLote[$chaveLote];
                $registrosEncontrados[$chaveLote] = array(
                    'posto' => $rowLoteOficio['posto'],
                    'lote' => $rowLoteOficio['lote'],
                    'quantidade' => (int)$rowLoteOficio['quantidade'],
                    'data_carga' => isset($rowLoteOficio['data_carga']) ? trim((string)$rowLoteOficio['data_carga']) : $datasSql[0],
                    'responsaveis' => isset($rowLoteOficio['responsaveis']) ? trim((string)$rowLoteOficio['responsaveis']) : $usuario,
                    'lacre_iipr' => $configLinha['lacre_iipr'],
                    'grupo_iipr' => $configLinha['grupo_iipr'],
                    'lacre_correios' => $configLinha['lacre_correios'],
                    'grupo_correios' => $configLinha['grupo_correios'],
                    'etiqueta_correios' => $configLinha['etiqueta_correios']
                );
            }

            $stmtInsLote = $pdo->prepare("INSERT INTO ciDespachoLotes (id_despacho, posto, lote, quantidade, data_carga, responsaveis, etiquetaiipr, grupo_iipr, etiquetacorreios, grupo_correios, etiqueta_correios)
                                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $lotesGravados = 0;
            foreach ($registrosEncontrados as $registroLote) {
                $stmtInsLote->execute(array(
                    $idDespacho,
                    $registroLote['posto'],
                    $registroLote['lote'],
                    $registroLote['quantidade'],
                    $registroLote['data_carga'],
                    $registroLote['responsaveis'],
                    ($registroLote['lacre_iipr'] > 0 ? $registroLote['lacre_iipr'] : null),
                    ($registroLote['grupo_iipr'] !== '' ? $registroLote['grupo_iipr'] : null),
                    ($registroLote['lacre_correios'] > 0 ? $registroLote['lacre_correios'] : null),
                    ($registroLote['grupo_correios'] !== '' ? $registroLote['grupo_correios'] : null),
                    ($registroLote['etiqueta_correios'] !== '' ? $registroLote['etiqueta_correios'] : null)
                ));
                $lotesGravados++;
            }

            $stmtDelMalote = $pdo->prepare("DELETE FROM ciMalotes WHERE leitura = ? AND tipo = 1");
            foreach ($etiquetasAntigas as $etiquetaAntiga) {
                $stmtDelMalote->execute(array($etiquetaAntiga));
            }

            $stmtInsMalote = $pdo->prepare("INSERT INTO ciMalotes (leitura, data, observacao, login, tipo, cep, sequencial, posto)
                                            VALUES (?, ?, ?, ?, 1, ?, ?, ?)");
            $etiquetasSalvas = 0;
            foreach ($etiquetasSnapshot as $dadosEtiqueta) {
                $etiqueta = preg_replace('/\D+/', '', (string)$dadosEtiqueta['etiqueta']);
                if (strlen($etiqueta) !== 35) {
                    continue;
                }
                $stmtDelMalote->execute(array($etiqueta));
                $stmtInsMalote->execute(array(
                    $etiqueta,
                    date('Y-m-d'),
                    'Ofício Correios #' . $idDespacho . ' gerado pela prévia da conferência',
                    $usuario,
                    substr($etiqueta, 0, 8),
                    substr($etiqueta, -5),
                    $dadosEtiqueta['posto']
            ...
            $_SESSION['id_despacho_correios'] = $idDespacho;
            ...
            die(json_encode(array(
                ...
                'linhas_gravadas' => count($snapshot['resumo']),
                'lotes_gravados' => $lotesGravados,
                'lotes_ignorados' => max(0, count($linhasPorLote) - count($registrosEncontrados)),

<?php
/* conferencia_pacotes.php — v9.8
 * NOVAS MELHORIAS DA v9.8:
 * 1. ✅ Mostra dia da semana (SEX/SÁB) discretamente nas datas de fim de semana
 * 2. ✅ Não exibe domingos como pendentes (exceto se houver produção)
 * 3. ✅ Corrige lógica: só mostra como pendente se houve produção E não foi conferido
 * 4. ✅ Verifica existência real de produção antes de marcar como não conferido
 * 
 * CORREÇÕES DA v9.6/9.7:
 * - Fundo verde aplicado corretamente ao carregar para lotes já conferidos
 * - Coluna mostra APENAS data/hora (lido_em), sem nome de usuário
 * - Divisão visual muito mais clara entre POUPA TEMPO e CORREIOS
 * - Som de conclusão corrigido para funcionar com lotes únicos
...
try {
    ...
    // Busca últimas 5 datas disponíveis COM DIA DA SEMANA
    $sql = "SELECT DISTINCT 
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        // DAYOFWEEK: 1=Domingo, 2=Segunda, ..., 6=Sexta, 7=Sábado
        $dia_semana_num = $row['dia_semana'];
        $label_dia = '';
        if ($dia_semana_num == 6) { // Sexta
            $label_dia = 'SEX';
        } elseif ($dia_semana_num == 7) { // Sábado
            $label_dia = 'SÁB';
    ...
    if (!empty($data_inicio) && !empty($data_fim)) {
        ...
        if (count($partes_inicio) == 3 && count($partes_fim) == 3) {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                elseif ($dia_semana_num == 7) $label_dia = 'SÁB';
...
<head>
    <meta charset="UTF-8">
    <title>Conferência de Pacotes v9.8</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Label na célula de data da tabela */
        .data-com-dia {
            display: flex;
            align-items: center;
            gap: 6px;
...
<body>
    <div class="container">
        <h1>📋 Conferência de Pacotes v9.8</h1>
...
if (!empty($pt_todos)) {
    ...
    foreach ($pt_todos as $p) {
        ...
        if (!empty($p['label_dia'])) {
            ...
            elseif ($p['label_dia'] == 'SÁB') $dia_class = 'sabado';

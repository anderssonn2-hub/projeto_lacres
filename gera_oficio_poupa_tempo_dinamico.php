...
try {
    $pdo = new PDO(
        'mysql:host=10.15.61.169;dbname=controle;charset=utf8mb4;connect_timeout=4',
        'controle_mat',
        '375256'
    );
...
if ($pdo && isset($_POST['ajax_resolver_codbar_pt'])) {
    ...
    if (!$ePoupaTempo) {
        die(json_encode(array('success' => false, 'erro' => 'Posto ' . $posto . ' (' . trim((string)$postoRow['nome']) . ') nao e Poupa Tempo. Entrega: ' . $entrega)));
    }

    // Usar comparacao inteira para aproveitar indice em lote e posto
    $loteInt = (int)$lote;
    ...
    $stmtCarga = $pdo->prepare("SELECT LPAD(CAST(lote AS CHAR),8,'0') AS lote,
                                       ...
                                       usuario
                                FROM ciPostosCsv
                                WHERE lote = ? AND posto = ?
                                ORDER BY CASE WHEN regional = ? THEN 0 ELSE 1 END,
                                         COALESCE(dataCarga, data) DESC
                                LIMIT 1");
    $stmtCarga->execute(array($loteInt, $postoInt, $regionalInt));
...
<audio id="audioPtBeep" src="beep_correio.mp3" preload="auto"></audio>
    ...
    function renderizarPostos() {
        ...
        for (var o = 0; o < estado.ordem.length; o++) {
            ...
            html += '<div class="posto-head">'
                + '<div><h3>POUPA TEMPO ' + codigo + ' - ' + escapar(grupo.nome) + '</h3>'
                + '<small>' + escapar(grupo.endereco || 'Endereço não informado') + '</small></div>'
                + '<div style="display:flex;align-items:center;gap:6px;">'
                + '<label style="font-size:11px;color:rgba(255,255,255,.75);white-space:nowrap;">Lacre PT</label>'
                + '<input type="text" value="' + lacrePtVal + '" '
            ...
            for (var i = 0; i < grupo.lotes.length; i++) {
                var item = grupo.lotes[i];
                html += '<tr class="' + (item.ativo ? '' : 'off') + (item.carregado ? ' conferido' : '') + '">';
                ...
                var dataTxt = formatarDataBr(item.data_carga) || '—';
                var naoCarregadoTag = (!item.carregado && !item.otimista) ? ' <span style="font-size:10px;color:#856404;background:#fff3cd;border:1px solid #ffc107;border-radius:3px;padding:1px 4px;font-weight:bold;">não carregado</span>' : '';
                html += '<td>' + dataTxt + naoCarregadoTag + '</td>';
    ...
    function adicionarLoteOtimista(codbar, lote, posto3, qtdBarra) {
        ...
        estado.postos[posto3].lotes.push({
            ...
            lote: lote,
            quantidade: qtdBarra,
            data_carga: document.getElementById('dataPadrao').value || '',
            responsaveis: '',
            carregado: false,
            ativo: true,
            otimista: true

...
if (isset($_POST['acao']) && $_POST['acao'] === 'salvar_oficio_correios') {
    try {
        ...
        // --- FONTE 1: snapshot_oficio JSON (campo 'posto' = data-posto-codigo da linha)
        $snap_json_mal = isset($_POST['snapshot_oficio']) ? trim((string)$_POST['snapshot_oficio']) : '';
        if ($snap_json_mal !== '') {
            $snap_items_mal = json_decode($snap_json_mal, true);
            if (is_array($snap_items_mal)) {
                foreach ($snap_items_mal as $snap_item) {
                    if (!is_array($snap_item)) continue;
                    $eti_raw = isset($snap_item['etiqueta_correios']) ? trim((string)$snap_item['etiqueta_correios']) : '';
                    if ($eti_raw === '') continue;
                    $eti_stripped = preg_replace('/\D+/', '', $eti_raw);
                    $eti_key = (strlen($eti_stripped) === 35) ? $eti_stripped : $eti_raw;
                    $posto_snap = isset($snap_item['posto']) ? trim((string)$snap_item['posto']) : '';
                    $posto_snap = preg_replace('/^p_/i', '', $posto_snap);
                    if (!isset($todasEtiquetas[$eti_key])) {
                        $todasEtiquetas[$eti_key] = ($posto_snap !== '') ? $posto_snap : null;
                    }
                }
            }
        }
        ...
        // --- FONTE 3: ciDespachoLotes do despacho recém-salvo (garantia de cobertura total)
        if (isset($id_desp) && $id_desp) {
            try {
                ...
                while ($rowEL = $stEtqLote->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ($eti_bd === '') continue;
                    $eti_bd_str = preg_replace('/\D+/', '', $eti_bd);
                    $eti_bd_key = (strlen($eti_bd_str) === 35) ? $eti_bd_str : $eti_bd;
                    if (!isset($todasEtiquetas[$eti_bd_key])) {
                        $posto_bd = trim((string)$rowEL['posto']);
                        $todasEtiquetas[$eti_bd_key] = ($posto_bd !== '') ? $posto_bd : null;
                    }
                }
...
if ($id_despacho_atual > 0 && $grupo_atual !== '') {
    if ($grupo_atual === 'correios') {
        $nome_pdf_titulo = $id_despacho_atual . "_correios_" . $data_atual;
    } elseif ($grupo_atual === 'poupatempo' || $grupo_atual === 'poupa tempo' || $grupo_atual === 'poupaTempo') {
        // Determinar se é metropolitano (postos 05-080) ou interior (postos 110-880)
        $subtipo_pt = 'poupa_tempo_metropolitano';
        try {
            $stmt_pt = $pdo_controle->prepare(
                "SELECT MIN(CAST(posto AS UNSIGNED)) AS min_posto FROM ciDespachoItens WHERE id_despacho = :id"
            );
            $stmt_pt->execute(array(':id' => $id_despacho_atual));
            $row_pt = $stmt_pt->fetch(PDO::FETCH_ASSOC);
            if ($row_pt && isset($row_pt['min_posto']) && (int)$row_pt['min_posto'] >= 110) {
                $subtipo_pt = 'poupa_tempo_interior';
            }
        } catch (Exception $e) { /* usa metropolitano como padrão */ }
        $nome_pdf_titulo = $id_despacho_atual . "_" . $subtipo_pt . "_" . $data_atual;
    } else {
...
<div class="assinaturas somente-impressao" style="display:flex;align-items:flex-end;gap:0;">
    <div class="assinatura-esquerda" style="flex:1;">
        <hr>
        <p>RESPONSÁVEL CELEPAR</p>
    </div>
    <div style="flex:0 0 auto;text-align:center;padding:0 20px;font-size:12px;">
        <p style="margin:0;">Data: <?php echo $data_geracao; ?></p>
    </div>
    <div class="assinatura-direita" style="flex:1;">
        <hr>
        <p>RESPONSÁVEL CORREIOS</p>
    </div>
</div>
